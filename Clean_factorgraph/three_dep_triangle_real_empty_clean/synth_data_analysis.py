...
def get_cont_cube(u_idx, v_idx, w_idx, matrix):
    ...
    #All present :
    table000 =np.sum(row_u_not*row_v_not*row_w_not)

    # v absent
    table010 = np.sum(row_u_not*row_v_present*row_w_not)

    # u absent
    table100 = np.sum(row_u_present*row_v_not*row_w_not)

    # u absent, v absent
    table110 = np.sum(row_u_present*row_v_present*row_w_not)

    # w absent
    table001 = np.sum(row_u_not*row_v_not*row_w_present)

    # v absent, w absent
    table011 = np.sum(row_u_not*row_v_present*row_w_present)

    # u absent, w absent
    table101 = np.sum(row_u_present*row_v_not*row_w_present)

    # all absent
    table111 = np.sum(row_u_present*row_v_present*row_w_present)
...
class Analyser():
    ...
    def analyse_asymptotic_2(self, alpha):
        ...
        extract_2_simplex_from_csv(self.savename + '_triangles_' + str(alpha)[2:] + '_pvalues.csv', alpha,
                                   self.savename + '_extracted_2-simplices_' + str(alpha)[2:])
    ...
    def analyse_asymptotic_for_triangle(self, alpha):
        one_simp_list = self.find_pairwise_p_values()

        link_count = 0
        for one_simp in one_simp_list:
            if one_simp[-1] < alpha:
                link_count += 1

        #if link_count == 3:

        #    cont_cube = get_cont_cube(0, 1, 2, self.bam)
        #    exp = iterative_proportional_fitting_AB_AC_BC_no_zeros(cont_cube)
        #    return chisq_test(cont_cube, exp)
