...
def total_func():
    ...
    for number_to_partition in range(30, 0, -2):
                                    ...
                                    for perm in permutations(np.array(l) * mask_permut):
                                        if perm not in perm_set:
                                            ...
                                            if np.any(sampled_table < 0):
                                                ...
                                                expected = iterative_proportional_fitting_AB_AC_BC_no_zeros(
                                                    sampled_table)
                                                p = chisq_test(sampled_table, expected)[1]
                                                # TODO : CHANGE comparison operator
                                                if p < 0.01:
                                                    switch = False
                                                    print('here', p, expected)
...
if __name__ == '__main__':
    ...
    # Iterate over a range of number to partition. The L1 norm is equal to 2*number_to_partition
    start = time.clock()
    for number_to_partition in range(25, 1, -1):
        ...
        print(time.clock()-start, len(l2), len(l3), len(l4), len(l5), len(l6), len(l7))

        simplest_part = np.concatenate(
            (np.array([number_to_partition]), np.array([number_to_partition, 0, 0, 0, 0, 0, 0]) * -1))
        perm_set = set()
        ...
        if switch is False:
            continue
        print('Before 2 :')
        for partition_list in [l2, l3, l4, l5, l6, l7]:
            print('Time : ', time.clock() - start, partition_list)
            start = time.clock()
            for partition in partition_list:
                total_partition = np.concatenate((np.array([number_to_partition]), partition * -1))
                while len(total_partition) < 8:
                    total_partition = np.concatenate((total_partition, np.array([0])))

                perm_set = set()
                for perm in permutations(total_partition):
                ...
                total_partition = total_partition * -1
                perm_set = set()
                for perm in permutations(total_partition):
        ...
        if switch is False:
            continue
        print('Time : ', time.clock() - start, partition_list)
        print('Before l2 + rest')
        for partition in l2:
            for partition_list in [l2, l3, l4, l5, l6]:
                print('Time : ', time.clock() - start, partition_list)
                start = time.clock()
                for partitidos in partition_list:
                    total_partition = np.concatenate((partition, partitidos * -1))
                    while len(total_partition) < 8:
                        total_partition = np.concatenate((total_partition, np.array([0])))
        ...
        print('Before l3 + rest')
        for partition in l3:
            for partition_list in [l3, l4, l5]:
                print('Time : ', time.clock() - start, partition_list)
                start = time.clock()
                for partitidos in partition_list:
                    total_partition = np.concatenate((partition, partitidos * -1))
    ...
    switch = True
    #Iterate over a range of number to partition. The L1 norm is equal to 2*number_to_partition
    for number_to_partition in range(56, 0, -2):
