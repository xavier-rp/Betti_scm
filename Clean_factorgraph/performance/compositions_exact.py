...
if __name__ == '__main__':
    ...
    for number_to_partition in range(2, 14, 2):
                                    ...
                                    for perm in permutations(np.array(l) * mask_permut):
                                        if perm not in perm_set:
                                            ...
                                            # because there are many permutations that are identical)
                                                ...
                                                if perm not in table_dictio.keys():
                                                    ...
                                                    expec = np.tile(expected, (nb_samples, 1)).reshape(nb_samples, 2, 2)
                                                    chisq = chisq_formula_vector(sample, expec)

                                                    p = sampled_chisq_test(sampled_table, expected, chisq)[1]
