...
if __name__ == '__main__':
    ...
    table_test = problist_to_table(probdist.prob_dist, 400)
    print(table_test)

    #exit()

    #with open('factorgraph_three-dep.pkl', 'wb') as output:
    ...
    #    np.save('/home/xavier/Documents/Projet/Betti_scm/Clean_factorgraph/three_dep_clean/data_100/bipartite_' + str(i), bipartite)
    ...
    for i in np.arange(0, 10000, 1):
        ...
        bam = np.load('/home/xavier/Documents/Projet/Betti_scm/Clean_factorgraph/three_dep_clean/data_100/bipartite_' +str(i) + '.npy')
        #print(bam.shape)
        ...
        analyser = Analyser(bam, '/home/xavier/Documents/Projet/Betti_scm/Clean_factorgraph/three_dep_clean/factorgraph_three-ind')
    ...
    rc('font', size=16)
    plt.figure(1)
    n, b, p = plt.hist(distance_list, bins=np.arange(0, 300,1), color='#00a1ffff')
    plt.xlabel('Distance $L_1$')
    ...
    plt.plot(np.arange(0.001, 0.101, 0.001), np.mean(np.array(list_of_compte_list), axis=0), color='#ff7f00ff',
             linewidth='2', label=r'Proportion d\textquotesingle acceptation')
    plt.fill_between(np.arange(0.001, 0.101, 0.001),
    ...
    for i in np.arange(0, 10000, 1):
        ...
        bam = np.load(
            '/home/xavier/Documents/Projet/Betti_scm/Clean_factorgraph/three_dep_clean/data_100/bipartite_' + str(
                i) + '.npy')
    ...
    #plt.plot(np.arange(0, 0.101, 0.001), np.arange(0, 0.101, 0.001), ls='--', color='#00a1ffff', label=r'$y = \alpha$')
    comptelist = []
    for alpha in np.arange(0.00, 0.1001, 0.001):
        compte = 0
        for pval in pval_list:
            if pval > alpha:
                compte += 1
        ...
        comptelist.append(compte)
    print(comptelist)
    comptelist = np.array(comptelist) / 111
    # plt.plot(np.arange(0, 0.101, 0.001), np.array(comptelist)/1000)
    rc('text', usetex=True)
    rc('font', size=16)
    plt.plot(np.arange(0.00, 0.101, 0.001), comptelist, color='#ff7f00ff',
    ...
    plt.plot(np.arange(0, 0.101, 0.001), 1 - np.array(comptelist) / 1000)
